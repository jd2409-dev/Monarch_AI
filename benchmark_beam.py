"""Benchmark the Guided Beam Search synthesizer on ARC training puzzles."""
...
from soma_mythos_ehra.arc3.beam_search import BeamConfig, beam_search_task


def benchmark(limit: int = 30) -> None:
    """Run beam search synthesizer on training puzzles and report accuracy."""
    ...
    for i, task in enumerate(tasks):
        try:
            t0 = time.time()
            result = beam_search_task(task, BeamConfig(
                beam_width=12,
                max_depth=4,
                timeout=8.0,
                top_k_primitives=8,
