"""Benchmark the DSL Program Synthesis engine on ARC training puzzles."""
...
from soma_mythos_ehra.arc3.dsl_synth import SynthConfig, synthesize_task


def benchmark(limit: int = 20) -> None:
    """Run DSL synthesizer on training puzzles and report accuracy."""
    ...
    for i, task in enumerate(tasks):
        try:
            t0 = time.time()
            result = synthesize_task(task, SynthConfig(
                max_depth=3,
                simulations=128,
                seed=42,
            ))
